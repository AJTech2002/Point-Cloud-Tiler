...
class oct_block:
    def __init__(self, parent, bounds, childrenBlocks=[], childrenPoints=[], level=0, id="root"):
        self.parent = parent
    ...
    def appendPoint(self, child, writer):
        ...
        if (len(self.childrenPoints) > 0 or self.count >= globalLimit - 2):
            ...
            for child in self.childrenPoints:
                lines.append(str(child['x']) + "," + str(child['y']) + "," + str(child['z']) + "," + str(child['r']) + "," + str(child['g']) + "," + str(child['b']) + "\n")
    ...
    def pointIsInBounds(self, point):
        return (
            (float(point['x']) >= float(self.bounds['min'][0]) and float(point['x']) <= float(self.bounds['max'][0]))
    ...
    def computeAverageGeometricError(self, detail):
        ...
        avgError = 0
        count = 0
        for (i, line) in enumerate(linecache.getlines(self.path)):
            if (i > detail):
                break
            count += 1
            point = self.convertStringToPoint(line)
            if (i == 0):
                continue
    ...
    def placeInChild(self, point):
        ...
        if (foundHome == False):
            print("Errors in bounds {} at : {}".format(len(errored), (str(point['x']) + "," + str(point['y']) + "," + str(point['z']))))
            gC += 1
    ...
    def outputToPnts(self):
        ...
        if (self.id == "root"):
            ...
            arr = np.concatenate((np.array(totalPoints, dtype=np.float32), np.array(totalColors, dtype=np.uint8)))
            writer.points_to_pnts('errored', arr.view(np.uint8), "output", True)
        ...
        if (len(self.childrenBlocks) == 0):
            ...
            arr = np.concatenate((np.array(totalPoints, dtype=np.float32).view(np.uint8), np.array(totalColors, dtype=np.uint8)))
    ...
    def deepSearchPoint(self, point, startTime):
        ...
        if (len(self.childrenBlocks) == 0):
            import time
            print("[" + str(self.level) + "]" + "Deep Search Time Elapsed : " + str(time.time() - startTime) + "\n")
    ...
    def print(self):
        ...
        print((" " * self.level * 2) + "Block : " + str(self.bounds['min']) + " : " + str(self.bounds['max']))
        for b in self.childrenBlocks:
            b.print()

        if (len(self.childrenBlocks) == 0):
            print((" " * self.level * 2) + "Children : " + str(self.getRealPointCount()))
            globalChildrenCount += self.getRealPointCount()
            # for p in self.childrenPoints:

        if (self.id == "root"):
            print("FINAL CHILDREN POINT COUNT {} vs {}".format(str(self.totalCount()[0]), str(self.totalCount()[1])))
